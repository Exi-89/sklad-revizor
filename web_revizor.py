...
import pypdf
import re
import pimport streamlit as st
import pypdf
import re
import pandas as pd
import os
...
# LOGO S INTERAKCÍ (Třída logo-img přidána přímo do tagu)
...
st.markdown("""
    <style>
    .main { background-color: #0b0e14; }
    .logo-container { display: flex; justify-content: center; padding: 20px 0px; }
...
# LOGO
repo_url = "https://raw.githubusercontent.com/Exi-89/sklad-revizor/main/logo_zzn.png"
st.markdown(f'<div class="logo-container"><img src="{repo_url}" class="logo-img"></div>', unsafe_allow_html=True)

# --- DATABÁZE ---
DB_FILE = "sklad_databaze.csv"

# vybrane je teď dict: { "položka": počet_kusů }
if 'vybrane' not in st.session_state: st.session_state.vybrane = {}
if 'db' not in st.session_state:
    if os.path.exists(DB_FILE):
        try: st.session_state.db = pd.read_csv(DB_FILE)['polozka'].tolist()
...
with l:
    ...
    for p in filtr:
        ...
        with c_row:
            ...
            if st.button(f"{ikona} {p}", key=f"r_{p}", use_container_width=True):
                if is_sel: del st.session_state.vybrane[p]
                else: st.session_state.vybrane[p] = 1 # Výchozí počet 1ks
                st.rerun()
        with c_del:
            if st.button("🗑️", key=f"d_{p}"):
                st.session_state.db.remove(p)
...
with r:
    ...
    for p in sorted(list(st.session_state.vybrane.keys())):
        ...
        # Zadávání množství
        nova_qty = c_qty.number_input("ks", min_value=1, value=st.session_state.vybrane[p], key=f"qty_{p}", label_visibility="collapsed")
        st.session_state.vybrane[p] = nova_qty
    ...
    if st.session_state.vybrane:
        st.divider()
        # Příprava textu pro kopírování včetně kusů
        txt_list = [f"{p} - {qty}ks" for p, qty in st.session_state.vybrane.items()]
        txt_copy = "\\n".join(sorted(txt_list))
