...
with col_b:
    search = st.selectbox("🔍 NAŠEPTÁVAČ (Filtr regálů):", [""] + st.session_state.db, index=0).lower()

# --- VSTUPY DAT ---
t1, t2 = st.tabs(["📄 PDF Převodka", "📝 Ruční přidání"])
with t1:
    up = st.file_uploader("Nahraj PDF soubor", type="pdf")
    ...
    if st.session_state.vybrane:
        ...
        components.html(f"""
            ...
            document.getElementById('cp').onclick = () => {{
                navigator.clipboard.writeText(`{txt_copy}`).then(() => alert('Seznam zkopírován!'));
            }};
            </script>
        """, height=70)
